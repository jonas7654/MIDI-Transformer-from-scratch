...
import config_for_fine_tuning as config
...
def read_datasets(split, data_dir, context_length, batch_size, rng, vo_size = None):
    # We recreate np.memmap every batch to avoid a memory leak, as per
    # https://stackoverflow.com/questions/45132940/numpy-memmap-memory-usage-want-to-iterate-once/61472122#61472122

    if split == 'train':
        data = np.memmap(os.path.join(data_dir, f'{vo_size}_train_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}.bin'), dtype=np.uint16, mode='r')
    elif split == 'test':
        data = np.memmap(os.path.join(data_dir, f'{vo_size}_test_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}.bin'), dtype=np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(data_dir, f'{vo_size}_val_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}.bin'), dtype=np.uint16, mode='r')
...
def compute_gradient(target, prediction, one_hot_lookup, alpha = config.reg_alpha, batch_size = None, padding_token_idx = 0):
    ...
    with Live(header_panel):
        ...
        while True:
            ...
            for micro_step in progress_step.track(range(config.gradient_accumulation_steps),
                ...
                # Get raw gradient
                raw_grad, target = compute_gradient(Y, logits, one_hot_lookup, alpha = model.reg_alpha, batch_size = model.batch_size)
