...
def read_datasets(split, data_dir, context_length, batch_size, rng):
    ...
    if split == 'train':
        data = np.memmap(os.path.join(data_dir, f'{config.vo_size}_train_full_data_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}.bin'), dtype=np.uint16, mode='r')
    elif split == 'test':
        data = np.memmap(os.path.join(data_dir, f'{config.vo_size}_test_full_data_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}.bin'), dtype=np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(data_dir, f'{config.vo_size}_val_full_data_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}.bin'), dtype=np.uint16, mode='r')
