...
def read_datasets(split, data_dir, context_length, batch_size, rng):
    ...
    if split == 'train':
        data = np.memmap(os.path.join(data_dir, f'{config.vo_size}_train_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}_random_padding_{config.use_random_padding_token}.bin'), dtype=np.uint16, mode='r')
    elif split == 'test':
        data = np.memmap(os.path.join(data_dir, f'{config.vo_size}_test_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}_random_padding_{config.use_random_padding_token}.bin'), dtype=np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(data_dir, f'{config.vo_size}_val_{config.tokenizer_name_str}_manual_tokens_{config.manually_set_sos_eos_trunc}_random_padding_{config.use_random_padding_token}.bin'), dtype=np.uint16, mode='r')
