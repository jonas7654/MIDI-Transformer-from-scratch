    ...
    with Live(Panel(Group(status, progress_step))):
        while True:
            ...
            for micro_step in progress_step.track(range(args.gradient_accumulation_steps),
                ...
                wandb.log({"Current local training loss": loss.item()})
            ...
            if iter_num % args.eval_interval == 0:
                ...
                print(f"Iteration {iter_num}, Train Loss: {losses_dataset['train'].item()}, Val Loss: {losses_dataset['val'].item()}")
                wandb.log({"train_loss": losses_dataset['train'].item(), "val_loss": losses_dataset['val'].item()})
            ...
            if iter_num % args.log_interval == 0:
                ...
                wandb.log({"step_loss": lossf, "step_time_ms": dt * 1000, "time": dt})
